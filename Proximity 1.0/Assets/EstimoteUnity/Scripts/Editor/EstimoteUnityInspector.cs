...
namespace OMobile.EstimoteUnity
{
	...
	public class EstimoteUnityInspector : Editor
	{
		...
		private void DrawHeading ()
		{
			...
			GUILayout.BeginHorizontal ();
			GUILayout.FlexibleSpace ();
			GUILayout.Label ("v1.2.1", GetSubTitleStyle ());
			GUILayout.FlexibleSpace ();
			GUILayout.EndHorizontal ();
	...
	/*	private void CheckIfPrefab ()
		{
			if (PrefabUtility.GetCorrespondingObjectFromSource (((EstimoteUnity)target).gameObject) != null && PrefabUtility.GetPrefabObject (((EstimoteUnity)target).gameObject) != null) {
				EditorGUILayout.Space ();
				GUI.color = Color.red;
